...
//    invokeAny();
  ...
  public static void invokeAny(){
    try {
      ...
      // executes syncrhonously and returns when one of the tasks has completed, all
  ...
  public static void invokeAll(){
    try {
      // submitting a collection of tasks
      // executes syncrhonously and returns when all of the tasks are completed
      // order is maintained i.e. the result for callables.get(0) goes into list.get(0)
      List<Future<String>> list = es.invokeAll(callables); // TimeUnit overload version exists
